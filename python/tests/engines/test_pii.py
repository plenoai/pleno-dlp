...
``RecognizerResult`` rows and assert that ``PiiEngine`` translates each
...
def _make_backend(
    ...
    *,
    ...
    return PiiEngine(language=language, entities=entities, client=client)
...
async def test_invalid_language_rejected() -> None:
    with pytest.raises(ValueError, match="language must be"):
        PiiEngine(language="fr")


@pytest.mark.asyncio
async def test_aclose_releases_owned_client() -> None:
    backend = PiiEngine(base_url="http://nowhere")
...
async def test_aclose_leaves_injected_client_alone() -> None:
    ...
    backend = PiiEngine(client=client)
