"""Built-in SaaS source connectors.
...
``import pleno_dlp``. Every connector here represents a SaaS provider
(github, slack, jira, …); the detection engines (native regex,
trufflehog, gitleaks, pii) live under ``pleno_dlp.engines`` and are
not connectors — they are stateless utilities a connector or the
pipeline applies to a Document.
"""
