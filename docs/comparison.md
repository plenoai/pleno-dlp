...
## Live re-measurement (auto-updated)

Everything below this box is the dated 2026-06-10 snapshot described
above, kept as a citable, hand-audited artifact (see "Reproducing").
This box is different: it is mechanically regenerated by
`go run ./bench/docsync` (issue #299) from `bench/harness`'s
machine-readable output every time pleno-dlp tags a release or a new
trufflehog/gitleaks release is detected — see
`.github/workflows/comparison-refresh.yml`. It only carries headline
recall counts and tool versions, never the prose/audit-trail/per-file
matrices below, which no automated diff can safely rewrite without
risking silent inconsistency with their own narrative.

<!-- BENCH:AUTO:START -->
**Generated:** 2026-09-21T10:45:44Z · **Trigger:** competitor release detected (trufflehog 3.97.5, gitleaks 8.30.1) · **Full run:** `bench/results/results.json` / `bench/results/results.md` (uploaded as a workflow artifact)

| Tool | Version measured |
|---|---|
| pleno-dlp | pleno-dlp version dev (none) |
| trufflehog | trufflehog 3.97.5 |
...
|--------|----------:|------------------:|----------------:|
| Detectors / rules | 615 | 870 | 222 |
| Live-verification capable | 548 (+67 unverified-by-design, see [`verify-coverage.md`](verify-coverage.md)) | most detectors (core design) | 0 — no verification subsystem |
...
7. **Detector breadth** — trufflehog ships 870 detector packages vs
   615 (long tail of niche SaaS providers).
...
- All claimed misses were re-verified by single-file re-scan with a
  positive control; the per-section re-scan commands and the
  "Reproducing" section below let any reader re-run those checks. The
  benchmark JSON they derive from is regenerated by `make bench` and
  uploaded as a CI workflow artifact — it is not committed to the repo,
  so the headline numbers are falsifiable by a fresh run rather than
  diffable against an in-repo snapshot.
