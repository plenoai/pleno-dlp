...
| propelauth | hardened | `[A-Za-z0-9]{40,}` | 256 | unknown — docs show only truncated examples; `pk_` prefix appears inconsistently (not anchored) | conservative fallback ([docs.propelauth.com apikey](https://docs.propelauth.com/reference/api/apikey); no trufflehog detector) | radius→64, arm regex, entropy 3.0, length kept | #135 |
| pusherchannels | hardened | `[A-Za-z0-9]{20}` | 256 | app_id `[0-9]{7}`; key & secret each **20-char lowercase** `[a-z0-9]` | authoritative: [trufflehog pusherchannelkey](https://github.com/trufflesecurity/trufflehog/tree/main/pkg/detectors/pusherchannelkey) (`[a-z0-9]{20}`) | charset→`[a-z0-9]{20}`, radius→64, arm regex, entropy 3.0 (3.5 would cull real low-hex secrets) | #135 |
| pushover | hardened | `[A-Za-z0-9]{30}` | 256 | **30**-char case-sensitive `[A-Za-z0-9]`, no prefix | authoritative: [pushover.net/api](https://pushover.net/api) ("30 characters long … [A-Za-z0-9]") | length 30 kept (now cited), radius→64, arm regex (requires `=`/`:` delimiter), entropy 3.5 | #135 |
| razorpay | hardened | `[A-Za-z0-9]{14,}` | 256 | key id `rzp_(test\|live)_`+base62 tail; secret **24**-char base62 | authoritative: [trufflehog razorpay](https://github.com/trufflesecurity/trufflehog/blob/main/pkg/detectors/razorpay/razorpay.go) (secret `{24}`) | key prefix-anchored `{14,}` (tail length not pinned — fixture has 16), secret→`{24}` + entropy 3.5, radius→64, arm regex | #135 |
| recurly | hardened | `[A-Za-z0-9]{32}` | 256 | unknown — docs/SDKs/gitleaks all omit length/prefix/charset | conservative fallback ([docs.recurly.com api-keys](https://docs.recurly.com/docs/api-keys); no trufflehog detector) | radius→64, arm regex, entropy 3.0, length kept (not re-pinned) | #135 |
| rekaai | hardened | `[A-Za-z0-9]{32,64}` | 256 | unknown — docs show only `your-api-key`/`REKA_API_KEY` placeholders (X-Api-Key header) | conservative fallback ([docs.reka.ai quick-start](https://docs.reka.ai/quick-start); no trufflehog detector) | radius→64, arm regex, entropy 3.0, length kept | #135 |
| ringcentral | hardened | `[A-Za-z0-9]{24,128}` | 256 | client_id **22**-char `[0-9A-Za-z_-]` (documented); client_secret undocumented | authoritative: [trufflehog ringcentral](https://github.com/trufflesecurity/trufflehog/blob/main/pkg/detectors/ringcentral/ringcentral.go) (`{22}`) | charset→`[0-9A-Za-z_-]{22,128}`, radius→64, arm regex, entropy 3.5 on both halves | #135 |
| rippling | hardened | `[A-Za-z0-9]{40,}` | 256 | unknown — opaque OAuth Bearer, no prefix/length/charset published | conservative fallback ([developer.rippling.com api-reference](https://developer.rippling.com/docs/rippling-api/a310f900b0f84-api-reference); no trufflehog detector) | radius→64, arm regex (incl. bare `RIPPLING_API`/`_AUTH`/`_CREDENTIAL` env forms), entropy 3.0, length kept | #135 |
| sageintacct | hardened | `[A-Za-z0-9]{12,32}` | 256 | unknown — Web Services sender_id/password "provisioned by Sage", no documented shape | conservative fallback ([developer.intacct.com web-services](https://developer.intacct.com/web-services/); no trufflehog detector) | radius→64, arm regex (`intacct\|sender\|user`), entropy 3.0, length kept | #135 |
| sapariba | hardened | `[A-Za-z0-9]{32,64}` | 256 | **32**-char base62, no prefix | authoritative: [SAP-samples ariba-extensibility](https://github.com/SAP-samples/ariba-extensibility-samples) (32-char apiKey examples) | length→`{32}`, radius→64, arm regex, entropy 3.5 | #135 |
| scoutapm | hardened | `[A-Za-z0-9]{16}` | 256 | no prefix, uppercase-alnum; length undocumented (examples conflict 20 vs 21) | conservative fallback ([scoutapm.com/docs/api](https://scoutapm.com/docs/api); no trufflehog detector) | radius→64, arm regex, entropy 3.0, **widened** `{16}`→`{16,30}` (recall) | #135 |
| semaphoreci | hardened | `[A-Za-z0-9]{40,80}` | 256 | unknown — docs show only `Authorization: Token` (trufflehog's `semaphore` targets a different SMS provider) | conservative fallback ([docs.semaphore.io reference/api](https://docs.semaphore.io/reference/api); no matching trufflehog detector) | radius→64, arm regex, entropy 3.0, length kept | #135 |
| sentinelone | hardened | `[A-Za-z0-9]{80,256}` | 256 | unknown — `Authorization: ApiToken` confirmed but no length/charset/prefix documented | conservative fallback (NinjaOne/Stitchflow integration docs; no trufflehog detector) | radius→64, arm regex, entropy 3.0, length kept | #135 |
| shodan | hardened | `[A-Za-z0-9]{32}` | 256 | **32**-char base62, no prefix | authoritative: [trufflehog shodankey](https://github.com/trufflesecurity/trufflehog/blob/main/pkg/detectors/shodankey/shodankey.go) (`{32}`) | length 32 kept (now cited), radius→64, arm regex, entropy 3.5 | #135 |
| signalwire | hardened | `[A-Za-z0-9]{24,128}` | 256 | two-part: project ID UUID; API token **50**-char `[0-9A-Za-z]` | authoritative: [trufflehog signalwire](https://raw.githubusercontent.com/trufflesecurity/trufflehog/main/pkg/detectors/signalwire/signalwire.go) (`{50}` + UUID) | radius→64, arm regex, entropy 3.5 on both halves; length **not** pinned (existing recall fixtures don't follow the 50/UUID shape — cardinal rule) | #135 |
