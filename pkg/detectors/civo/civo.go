...
var tokenRe = sync.OnceValue(func() *regexp.Regexp { return regexp.MustCompile(`\b([A-Za-z0-9]{50})\b`) })
...
// key takes.
var armRe = sync.OnceValue(func() *regexp.Regexp { return regexp.MustCompile(`(?i)civo[_\-]?(api[_\-]?)?(token|key|secret)`) })
