...
func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Deepgram {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("deepgram_api_key=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without deepgram keyword, got %d", len(res))
	}
}

func TestFromData_Dedup(t *testing.T) {
	body := []byte("deepgram=" + dummyToken + "\ndg_api=" + dummyToken)
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 1 {
