...
var tokenRe = regexp.MustCompile(`(?i)hanko[_\.\-]?(?:api[_\.\-]?key|admin[_\.\-]?key|token|secret)\s*[:=]\s*["']?([A-Za-z0-9_\-\.]{32,200})["']?`)

type Scanner struct{}

func (Scanner) Type() detectors.DetectorType { return detectors.Hanko }

func (Scanner) Keywords() []string { return []string{"hanko"} }
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(apiBase, "/")+"/webhooks", nil)
	if err != nil {
		return false, err
	}
