...
func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.DNSimple {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("dnsimple_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without dnsimple keyword, got %d", len(res))
	}
}

func TestFromData_Dedup(t *testing.T) {
	body := []byte("dnsimple=" + dummyToken + "\ndnsimple_api=" + dummyToken)
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 1 {
