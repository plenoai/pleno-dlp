...
// pairs near the `eloqua` keyword. Verified via /api/REST/2.0/system/user/current
// on the per-pod host (`secure.p<NN>.eloqua.com`). Unverified-by-default —
// the pod host isn't in the chunk; verify only fires when an apiBase override
// is supplied. Raw carries the client_id, RawV2 the secret.
package eloqua
...
var contextKeywords = []string{"eloqua"}
