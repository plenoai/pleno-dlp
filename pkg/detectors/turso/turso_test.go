...
const dummyToken = "eyJhbGciOiJSUzI1NiJ9.abcdefABCDEF0123456789abcdefABCDEF.signature"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Turso {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("TURSO_AUTH_TOKEN=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without turso keyword, got %d", len(res))
	}
}
