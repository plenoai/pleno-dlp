...
const dummyToken = "live_abcdefghijklmnopqrstuvwxyz0123456789"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Chargebee {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("CHARGEBEE_API_KEY=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without chargebee keyword, got %d", len(res))
	}
}
