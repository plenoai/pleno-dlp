// Package qdrant detects Qdrant Cloud API keys — long base64url tokens near
// the `qdrant` keyword. Qdrant Cloud assigns a per-cluster host
// (`<cluster-id>.<region>.aws.cloud.qdrant.io`), so verify requires apiBase
...
func (Scanner) Type() detectors.DetectorType { return detectors.Qdrant }
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(apiBase, "/")+"/collections", nil)
	if err != nil {
		return false, err
	}
