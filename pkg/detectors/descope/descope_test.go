...
const dummyToken = "K2abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGH"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Descope {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("DESCOPE_MGMT_KEY=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without descope keyword, got %d", len(res))
	}
}
