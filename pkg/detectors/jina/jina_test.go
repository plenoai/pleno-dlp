...
const dummyToken = "jina_abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Jina {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("JINA_API_KEY=" + dummyToken)
...
func TestFromData_NoMatch(t *testing.T) {
	body := []byte("token=ji_not_a_jina_key_random_text_long_enough_xx")
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without jina_ prefix, got %d", len(res))
	}
}
