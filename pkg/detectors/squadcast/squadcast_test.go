...
const dummyToken = "abcdef0123456789ABCDEF0123456789abcdefABCDef"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Squadcast {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("SQUADCAST_TOKEN=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without squadcast keyword, got %d", len(res))
	}
}
