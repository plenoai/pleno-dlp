...
const dummyToken = "nk-abcdefghijklmnopqrstuvwxyz0123456789ABCDEF"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Nomic {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("NOMIC_API_KEY=" + dummyToken)
...
func TestFromData_NoMatch(t *testing.T) {
	body := []byte("token=ak-not_a_nomic_key_just_random_text_here_x")
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without nk- prefix, got %d", len(res))
	}
}
