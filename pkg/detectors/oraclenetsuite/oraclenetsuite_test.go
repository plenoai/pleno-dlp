...
const dummySecret = "fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.OracleNetSuite {
...
func TestFromData_Found(t *testing.T) {
	body := []byte(`netsuite_token_id="` + dummyID + `"
netsuite_token_secret="` + dummySecret + `"`)
	...
	found := false
	for _, r := range res {
		if string(r.RawV2) == dummyID+":"+dummySecret {
			found = true
		}
	}
	if !found {
		t.Fatal("expected RawV2 with id+secret")
	}
}

func TestFromData_None(t *testing.T) {
	body := []byte("ID=admin SECRET=hunter2")
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
