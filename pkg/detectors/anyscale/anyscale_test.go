...
const dummyToken = "esct_aBcDeFgHiJkLmNoPqRsTuVwXyZ0123456789ABCDEFGH"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Anyscale {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("ANYSCALE_API_KEY=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without anyscale keyword, got %d", len(res))
	}
}
