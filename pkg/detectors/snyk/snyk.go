...
var contextKeywords = []string{"snyk", "snyk_token", "snyk_api_token"}

type Scanner struct{}

func (Scanner) Type() detectors.DetectorType { return detectors.Snyk }

func (Scanner) Keywords() []string { return []string{"snyk"} }
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) ([]detectors.Result, error) {
	...
	for _, h := range hits {
		...
		// UUID alone is too generic — keyword gate is mandatory.
		if !nearKeyword(lower, h[2], h[3]) {
			continue
		}
		seen[token] = struct{}{}
		res := detectors.Result{
			DetectorType: detectors.Snyk,
			Raw:          []byte(token),
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	// Snyk uses `Authorization: token <key>`, not Bearer.
