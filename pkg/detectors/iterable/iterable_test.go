...
func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Iterable {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("ITERABLE_API_KEY=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without iterable keyword, got %d", len(res))
	}
}
...
func TestVerify_OK(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Api-Key") != dummyToken {
			t.Errorf("header mismatch: %q", r.Header.Get("Api-Key"))
		}
