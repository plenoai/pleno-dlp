...
var apiBase = "https://api.iterable.com"
...
func (Scanner) Type() detectors.DetectorType { return detectors.Iterable }

func (Scanner) Keywords() []string { return []string{"iterable"} }
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) ([]detectors.Result, error) {
	...
	for _, h := range hits {
		...
		res := detectors.Result{
			DetectorType: detectors.Iterable,
			Raw:          []byte(token),
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	defer cancel()
	endpoint := strings.TrimRight(apiBase, "/") + "/api/users/byEmail?" + url.Values{"email": {"verify@example.com"}}.Encode()
	...
	case http.StatusOK, http.StatusNotFound:
		// 404 means "user not found" — auth was accepted.
		return true, nil
