// Package hetzner detects Hetzner Cloud API tokens (`<base64url>{64}` near
// `hcloud` / `hetzner` keyword). Verified via /v1/servers on api.hetzner.cloud
// using Bearer auth.
package hetzner
...
// Hetzner Cloud API tokens are documented as 64 char base64url-ish.
var tokenRe = regexp.MustCompile(`\b([A-Za-z0-9]{64})\b`)

var contextKeywords = []string{"hcloud", "hetzner", "hetzner_token"}
