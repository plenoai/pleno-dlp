// Package cerebras detects Cerebras Cloud inference API keys
// (`csk-<base62>`) and verifies them against /v1/models with Bearer auth.
package cerebras
...
func (Scanner) Type() detectors.DetectorType { return detectors.Cerebras }
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) ([]detectors.Result, error) {
	...
	for _, m := range matches {
		...
		res := detectors.Result{
			DetectorType: detectors.Cerebras,
			Raw:          []byte(token),
