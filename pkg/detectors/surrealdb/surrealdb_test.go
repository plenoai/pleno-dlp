...
const dummyToken = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0In0.signaturepartabcdefghij"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.SurrealDB {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("surrealdb_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without surrealdb keyword, got %d", len(res))
	}
}
