...
const (
	dummyTest = "sk_test_abcdefghijklmnopqrstuvwxyzABCDEF0123456789"
	dummyLive = "sk_live_abcdefghijklmnopqrstuvwxyzABCDEF0123456789"
)

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.WorkOS {
...
func TestFromData_Test(t *testing.T) {
	body := []byte("WORKOS_API_KEY=" + dummyTest)
...
func TestFromData_Live(t *testing.T) {
	body := []byte("workos_secret=" + dummyLive)
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) == 0 {
...
func TestFromData_NoKeyword(t *testing.T) {
	res, _ := Scanner{}.FromData(context.Background(), false, []byte("KEY="+dummyTest))
	if len(res) != 0 {
