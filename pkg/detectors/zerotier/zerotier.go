// Package zerotier detects ZeroTier Central API tokens (32-char
// alnum). Surface only when a `zerotier` keyword is in the same chunk
// to keep the broad alnum shape from triggering universally. Verified
// via /api/v1/status on my.zerotier.com with the Authorization Bearer
// header.
package zerotier
...
var contextKeywords = []string{"zerotier"}
