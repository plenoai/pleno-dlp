...
// Harbor robot-account CLI secrets are generated by the server's
// GenerateRandomString() == GenerateRandomStringWithLen(32): exactly 32
// chars over the 62-char base62 alphabet [A-Za-z0-9], no prefix.
// Authoritative: goharbor/harbor src/common/utils/utils.go (len=32,
// charset abc..ABC..0-9) wired in src/controller/robot/controller.go
// (`pwd = utils.GenerateRandomString()`). Server-side IsValidSec() also
...
// but are not random secrets (e.g. padded placeholders, repeated chars).
// Harbor secrets are 32 base62 chars with enforced lower/upper/digit
// variety, so their Shannon entropy sits well above 3.5 bits/char.
