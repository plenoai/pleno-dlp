...
const dummyToken = "eyJhbGciOiJSUzI1NiJ9.eyJzdWIiOiJ0ZXN0In0.signaturepartabcdefghij"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.DocuSign {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("docusign_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without docusign keyword, got %d", len(res))
	}
}
