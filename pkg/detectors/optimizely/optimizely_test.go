...
const dummyToken = "2:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMN0123"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Optimizely {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("optimizely_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without optimizely keyword, got %d", len(res))
	}
}
