...
// strings near the `apollo` keyword. Verified via /v1/auth/health on
...
var contextKeywords = []string{"apollo"}
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(apiBase, "/")+"/v1/auth/health", nil)
	if err != nil {
		return false, err
	}
