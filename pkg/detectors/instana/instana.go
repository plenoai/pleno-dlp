...
var contextKeywords = []string{"instana"}

type Scanner struct{}

func (Scanner) Type() detectors.DetectorType { return detectors.Instana }

func (Scanner) Keywords() []string { return []string{"instana"} }
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) ([]detectors.Result, error) {
	...
	for _, h := range hits {
		...
		res := detectors.Result{
			DetectorType: detectors.Instana,
			Raw:          []byte(token),
...
func (Scanner) Verify(ctx context.Context, secret string) (bool, error) {
	...
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(apiBase, "/")+"/api/instana/version", nil)
	if err != nil {
		return false, err
	}
