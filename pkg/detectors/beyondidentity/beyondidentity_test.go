...
func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.BeyondIdentity {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("beyondidentity_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without beyondidentity keyword, got %d", len(res))
	}
}

func TestFromData_Dedup(t *testing.T) {
	body := []byte("beyondidentity=" + dummyToken + "\nbyndid=" + dummyToken)
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 1 {
