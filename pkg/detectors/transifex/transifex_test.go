...
const dummyToken = "1/abcdefghijklmnopqrstuvwxyz0123456789ABCD"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.Transifex {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("transifex_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without transifex keyword, got %d", len(res))
	}
}
