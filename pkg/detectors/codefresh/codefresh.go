// Package codefresh detects Codefresh CI/CD API keys (long base64url near
// `codefresh` keyword). Verified via /api/user on g.codefresh.io with the
// documented `Authorization: <token>` header (no Bearer prefix).
package codefresh
...
func (Scanner) Type() detectors.DetectorType { return detectors.Codefresh }
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) ([]detectors.Result, error) {
	...
	for _, h := range hits {
		...
		res := detectors.Result{
			DetectorType: detectors.Codefresh,
			Raw:          []byte(token),
