...
func TestFromData_Positive(t *testing.T) {
	res, err := Scanner{}.FromData(context.Background(), false, []byte("FIREBASE_KEY="+dummy))
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if len(res) != 1 {
...
func TestFromData_Negative(t *testing.T) {
	res, _ := Scanner{}.FromData(context.Background(), false, []byte("AIzaShort"))
	if len(res) != 0 {
...
func TestRedact(t *testing.T) {
	...
	if !strings.HasPrefix(r, "AIzaSy01") {
