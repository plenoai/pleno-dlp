...
// near `jenkinsx` / `jx_` keyword. Self-hosted: per-installation host is
// not in the chunk, so this detector is unverified-by-design (apiBase
...
// Jenkins X tokens are documented as 40+ char alphanumeric.
var tokenRe = regexp.MustCompile(`\b([A-Za-z0-9]{40,80})\b`)

var contextKeywords = []string{"jenkinsx", "jenkins_x", "jx_token", "jx_api"}
