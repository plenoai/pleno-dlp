...
const dummyToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMTIzIiwicm9sZSI6Im93bmVyIn0.dummysig0123456789ABCDEF"

func TestType(t *testing.T) {
	if (Scanner{}).Type() != detectors.MotherDuck {
...
func TestFromData_Found(t *testing.T) {
	body := []byte("motherduck_token=" + dummyToken)
...
func TestFromData_NoKeyword(t *testing.T) {
	...
	res, _ := Scanner{}.FromData(context.Background(), false, body)
	if len(res) != 0 {
		t.Fatalf("expected 0 without motherduck keyword, got %d", len(res))
	}
}
