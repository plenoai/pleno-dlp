...
func scanCodebase(ctx context.Context, cfg Config, emit Emit) error {
	...
	if err := scanCodebaseTickets(ctx, cli, project, maxBytes, emit); err != nil {
		return err
	}
	return scanCodebaseMergeRequests(ctx, cli, project, repo, maxBytes, emit)
}
...
func scanCodebaseTickets(ctx context.Context, cli *codebaseClient, project string, maxBytes int64, emit Emit) error {
	for page := 1; ; page++ {
		...
		for _, ticket := range list.Tickets {
			...
			for _, note := range notes.Notes {
				if err := emitCodebasePart(note.Content, maxBytes, sources.ForgeMeta{
					Provider:   "codebase",
					Repository: project,
...
func scanCodebaseMergeRequests(ctx context.Context, cli *codebaseClient, project, repo string, maxBytes int64, emit Emit) error {
	...
	for _, summary := range list.MergeRequests {
		...
		for i, comment := range mr.Comments {
			if err := emitCodebasePart(comment.Content, maxBytes, sources.ForgeMeta{
				Provider:   "codebase",
...
func emitCodebasePart(text string, maxBytes int64, meta sources.ForgeMeta, emit Emit) error {
