...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **587 detectors**
  built-in (583 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
587 built-in detectors. Every secret detector that can confirm against
