...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **557 detectors**
  built-in (553 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
557 built-in detectors. Every secret detector that can confirm against
