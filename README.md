...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **302 detectors**
  built-in (298 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
302 built-in detectors. Every secret detector that can confirm against
