...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **527 detectors**
  built-in (523 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
527 built-in detectors. Every secret detector that can confirm against
