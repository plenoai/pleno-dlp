...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **362 detectors**
  built-in (358 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
362 built-in detectors. Every secret detector that can confirm against
