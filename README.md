...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **437 detectors**
  built-in (433 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
437 built-in detectors. Every secret detector that can confirm against
