...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **197 detectors**
  built-in (193 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
197 built-in detectors. Every secret detector that can confirm against
