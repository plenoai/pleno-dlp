...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **542 detectors**
  built-in (538 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
542 built-in detectors. Every secret detector that can confirm against
