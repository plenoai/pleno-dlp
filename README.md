...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **347 detectors**
  built-in (343 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
347 built-in detectors. Every secret detector that can confirm against
