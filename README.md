...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **272 detectors**
  built-in (268 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
272 built-in detectors. Every secret detector that can confirm against
