...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **212 detectors**
  built-in (208 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
212 built-in detectors. Every secret detector that can confirm against
