...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **182 detectors**
  built-in (178 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
182 built-in detectors. Every secret detector that can confirm against
