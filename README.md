...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **452 detectors**
  built-in (448 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
452 built-in detectors. Every secret detector that can confirm against
