...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **227 detectors**
  built-in (223 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
227 built-in detectors. Every secret detector that can confirm against
