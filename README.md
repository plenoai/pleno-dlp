...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **512 detectors**
  built-in (508 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
512 built-in detectors. Every secret detector that can confirm against
