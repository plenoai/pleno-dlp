...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **332 detectors**
  built-in (328 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
332 built-in detectors. Every secret detector that can confirm against
