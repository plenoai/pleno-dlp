...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **482 detectors**
  built-in (478 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
482 built-in detectors. Every secret detector that can confirm against
