...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **167 detectors**
  built-in (163 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
167 built-in detectors. Every secret detector that can confirm against
