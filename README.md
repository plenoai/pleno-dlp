...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **377 detectors**
  built-in (373 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
377 built-in detectors. Every secret detector that can confirm against
