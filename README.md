...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **317 detectors**
  built-in (313 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
317 built-in detectors. Every secret detector that can confirm against
