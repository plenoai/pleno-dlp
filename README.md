...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **497 detectors**
  built-in (493 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
497 built-in detectors. Every secret detector that can confirm against
