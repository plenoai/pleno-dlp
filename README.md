...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **287 detectors**
  built-in (283 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
287 built-in detectors. Every secret detector that can confirm against
