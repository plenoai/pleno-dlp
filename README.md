...
- **Go binary** (`cmd/pleno-dlp/`, this README) — filesystem, local git
  ...
  decoder pipeline, per-host verify rate limiter. **392 detectors**
  built-in (388 secrets + 4 PII). Tag pattern `vX.Y.Z`.
...
392 built-in detectors. Every secret detector that can confirm against
